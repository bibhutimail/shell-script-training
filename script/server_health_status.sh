#! /bin/bash
#Author : - Bibhuti Narayan Veersatech
#Date : - 25 sept. 2022

#Checking if user UID not zero
if [ $EUID != 0 ]
then
...
#Declaring variables
#set -x
os_name=`uname -a | awk {'print$1'} | cut -f2 -d'-'`
upt=`uptime | awk {'print$3'} | cut -f1 -d','`
ip_add=`ifconfig | grep "inet" |head -1 |awk {'print$2'}`
num_proc=`ps -ef | wc -l`
root_fs_pc=`df -h /dev/sdc | tail -1 | awk '{print$5}'`
total_root_size=`df -h /dev/sdc | tail -1 | awk '{print$2}'`
...
os_version=`uname -a | cut -f2 -d'~' | awk {'print$3'}`
#Creating a directory if it doesn't exist to store reports first, for easy maintenance.
if [ ! -d ./health_reports ]
then
  mkdir ./health_reports
fi
html="./health_reports/Server-Health-Report-`hostname`-`date +%y%m%d`-`date +%H%M`.html"
...
echo "<h3><legend>Script authored by Bibhuti Narayan Veersatech</legend></h3>" >> $html
echo "</center>" >> $html
echo "</fieldset>" >> $html
echo "<br>" >> $html
echo "<center>" >> $html
...
echo "</body>" >> $html
echo "</html>" >> $html
echo "Report has been generated in ./health_reports with file-name = $html. Report has also been sent to $email_add."
#Sending Email to the user
#cat $html | mail -s "`hostname` - Daily System Health Report" -a "MIME-Version: 1.0" -a "Content-Type: text/html" -a "From: Bibhuti Narayan <bibhuti@veersatech.com>" $email_add
